...
public class OrderService implements IOrderService {
    ...
    public Pager<Order> getSpecialPager(OrderSearcher searcher, int pageNo, int pageSize)
    {
        ...
        map.forEach((k, v) -> v = v.stream().parallel().sorted(Comparator.<Order>comparingLong(x -> x.getSubmitTime().getTime()).reversed()).collect(toList()));
        List<List<Order>> mapList = new ArrayList<>();
        map.forEach((k, v) -> mapList.add(v));
        List<List<Order>> collect = mapList.stream().sorted(Comparator.<List<Order>>comparingLong(x -> x.get(0).getSubmitTime().getTime()).reversed()).collect(toList());
